...
const YOUTUBE_WATCH_TIME_CATEGORY = "youtube-watch-time";

export default function YouTubeWatchTimeClient() {
  ...
  const watchTimeServices = useMemo(() => getServicesByCategory(YOUTUBE_WATCH_TIME_CATEGORY), []);
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <div className="mb-10">
              ...
              <p className="text-kenya-white/60 text-lg max-w-2xl">
                Boost your YouTube watch time hours for monetization. Choose from high-retention long-video, standard speed, or fast-start mixed duration packages.
              </p>
            </div>

            {/* Quick Order Cards */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
              {watchTimeServices.map((service) => (
                <Link
                  key={service.id}
                  href={`/order/youtube-watch-time/${service.id}`}
            ...
            <div className="bg-kenya-white/5 border border-kenya-white/10 rounded-2xl p-6 mb-6">
              <label className="block text-sm font-semibold text-kenya-white/70 mb-3 uppercase tracking-wider">
                Select YouTube Watch Time Service
              </label>
              <select
              ...
              <div className="bg-kenya-green/10 border border-kenya-green/30 rounded-2xl p-6 mb-6">
                ...
                <div className="flex flex-wrap items-center gap-3 mb-4">
                  <span className="inline-flex items-center gap-1.5 bg-kenya-black/60 text-kenya-white text-xs px-3 py-1.5 rounded-lg border border-kenya-white/10">
                    ⚡ Rate: KES {selectedService.rate.toFixed(2)} / hour
                  </span>
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder={`Enter quantity (${selectedService.min.toLocaleString()} - ${selectedService.max.toLocaleString()} hours)`}
