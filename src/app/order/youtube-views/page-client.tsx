...
const YOUTUBE_VIEWS_CATEGORY = "youtube-views";

export default function YouTubeViewsClient() {
  ...
  const viewsServices = useMemo(() => getServicesByCategory(YOUTUBE_VIEWS_CATEGORY), []);
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <div className="mb-10">
              ...
              <p className="text-kenya-white/60 text-lg max-w-2xl">
                Boost your YouTube videos with high-retention views. Choose from monetizable, fast-speed, or Shorts-specific packages.
              </p>
            </div>

            {/* Quick Order Cards */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
              {viewsServices.map((service) => (
                <Link
                  key={service.id}
                  href={`/order/youtube-views/${service.id}`}
              ...
              <div className="bg-kenya-green/10 border border-kenya-green/30 rounded-2xl p-6 mb-6">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    {selectedService.monetizable && (
                      <span className="bg-kenya-green text-kenya-black text-xs font-bold px-2.5 py-1 rounded-lg">
                        💰 Monetizable
                      </span>
                    )}
                ...
                <div className="flex flex-wrap items-center gap-3 mb-4">
                  <span className="inline-flex items-center gap-1.5 bg-kenya-black/60 text-kenya-white text-xs px-3 py-1.5 rounded-lg border border-kenya-white/10">
                    ⚡ Rate: KES {selectedService.rate.toFixed(2)} / view
                  </span>
