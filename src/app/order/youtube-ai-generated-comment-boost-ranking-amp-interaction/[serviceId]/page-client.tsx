...
export default function FulfillmentClient({ serviceId }: FulfillmentClientProps) {
  ...
  const quantityError = useMemo(() => {
    ...
    if (quantityNum < service.min) return `Minimum quantity is ${service.min}`;
  ...
  }, [service, link, quantityNum, quantityError, total, walletBalance, user, openAuth]);
        ...
        <main className="flex-1">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <nav className="flex items-center gap-2 text-sm text-kenya-white/50 mb-6">
              ...
              <span>/</span>
              <Link href="/order/youtube-ai-generated-comment-boost-ranking-amp-interaction" className="hover:text-kenya-green transition-colors">YouTube AI Comments</Link>
              <span>/</span>
            ...
            <div className="bg-kenya-white/5 border border-kenya-white/10 rounded-2xl p-6 sm:p-8 mb-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-kenya-black/40 rounded-xl p-4 border border-kenya-white/5">
                  ...
                  <p className="text-kenya-white font-bold text-sm">KES {service.rate.toFixed(2)} / comment</p>
                </div>
                <div className="bg-kenya-black/40 rounded-xl p-4 border border-kenya-green/30">
                  ...
                  <p className="text-kenya-green font-bold text-lg">KES {(service.rate * HAPPY_HOUR_DISCOUNT).toFixed(2)} / comment</p>
                </div>
              </div>
            ...
            <div className="bg-kenya-white/5 border border-kenya-white/10 rounded-2xl p-6 sm:p-8 mb-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-kenya-white/70 mb-2">
                    💬 Video Niche / Context <span className="text-kenya-white/40">(optional)</span>
                  </label>
                  <textarea
                    value={niche}
                    onChange={(e) => setNiche(e.target.value)}
                    placeholder="Describe your video niche or topic for tailored AI comment generation...&#10;Example: Kenyan tech review, Gen Z lifestyle vlog, crypto trading tips"
                    rows={3}
                    className="w-full bg-kenya-black border border-kenya-white/20 rounded-xl px-4 py-3.5 text-kenya-white placeholder-kenya-white/30 focus:outline-none focus:border-kenya-green focus:ring-1 focus:ring-kenya-green transition-all resize-none"
                  />
                  <p className="text-kenya-white/40 text-xs mt-2">Help the AI engine generate contextually relevant comments for your video</p>
                </div>
                ...
                {quantityNum > 0 && !quantityError && (
                  <div className="bg-kenya-black/60 rounded-xl p-5 border border-kenya-white/10 space-y-3">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-kenya-white font-medium">{quantityNum.toLocaleString()} comments</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-kenya-white/60">Unit Rate</span>
                      <span className="text-kenya-white font-medium">KES {service.rate.toFixed(2)}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
